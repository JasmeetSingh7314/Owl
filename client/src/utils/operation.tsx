...
const contractAddress='KT19YBGhmge3upmDXFJVKmJrWGoazhtHZSjR';
...
export const mintNFT = async (nftDetails:nftObject) => {
  try {
    ...
    const contract = await tezos.wallet.at('KT1CBM3xRtxt6VXjtXfhZ7hpucMgbNNdHizB');
    
   
    console.log()
    const bytes = jsonToHexBytes(nftDetails.metadata.url.replace('"',''))
...
export const transferNFT=async()=>{
  try{
    const contractInstance=await tezos.wallet.at('KT1Bhxk1pcbMqu9kf3brpbZMTaEHVWGmcPsy');
    const op=await contractInstance.methods.fa2_transfer().send();
  }catch(error){
    throw error;
  }
}
