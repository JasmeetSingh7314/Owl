...
export const mintNFT = async (nftDetails:nftObject) => {
  try {
    ...
    const contract = await tezos.wallet.at('KT1RBiovwqPGYxWFz4kcsHCHc6MCvTYWHTG5');
    
   

    const bytes = jsonToHexBytes(nftDetails.metadata)
...
export const fetchData = async () => {
		...
		try {
			...
			for (let i = 0; i < d1.length; i++) {
				const s = hex2buf(d2[i].value.token_info[""]);
					
 
				const res:any = await fetch("https://ipfs.io/ipfs/" + s);
